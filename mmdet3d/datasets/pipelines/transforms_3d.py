...
from ...core.bbox import BaseInstance3DBoxes # Make sure this is imported
...
class RandomFlip3D(object):
    ...
    Args:
        ...
        sync_2d (bool): Unused in this implementation but kept for
    ...
    def __init__(self,
                 ...
                 sync_2d=True,  # Kept for API compatibility, but not used
                 **kwargs):
        super(RandomFlip3D, self).__init__()
        self.flip_ratio_bev_horizontal = flip_ratio_bev_horizontal
        self.flip_ratio_bev_vertical = flip_ratio_bev_vertical
        if flip_ratio_bev_horizontal is not None:
            assert 0 <= flip_ratio_bev_horizontal <= 1
        if flip_ratio_bev_vertical is not None:
            assert 0 <= flip_ratio_bev_vertical <= 1
    ...
    def random_flip_data_3d(self, input_dict, direction='horizontal'):
        ...
        # Flip 3D bounding boxes
        if 'gt_bboxes_3d' in input_dict:
            input_dict['gt_bboxes_3d'].flip(direction)
        
        # Flip occupancy grid
        if 'voxel_semantics' in input_dict:
            if direction == 'horizontal':
                # Horizontal flip corresponds to flipping the Y-axis in BEV
                input_dict['voxel_semantics'] = np.flip(
                    input_dict['voxel_semantics'], axis=1).copy()
                if 'mask_camera' in input_dict:
                    input_dict['mask_camera'] = np.flip(
                        input_dict['mask_camera'], axis=1).copy()
            else: # vertical
                # Vertical flip corresponds to flipping the X-axis in BEV
                input_dict['voxel_semantics'] = np.flip(
                    input_dict['voxel_semantics'], axis=0).copy()
                if 'mask_camera' in input_dict:
                    input_dict['mask_camera'] = np.flip(
                        input_dict['mask_camera'], axis=0).copy()

    def __call__(self, input_dict):
        """Call function to flip points, bboxes, and occupancy grid.
        ...
        """
        # Determine whether to flip horizontally
        pcd_horizontal_flip = np.random.rand() < self.flip_ratio_bev_horizontal
        
        # Determine whether to flip vertically
        pcd_vertical_flip = np.random.rand() < self.flip_ratio_bev_vertical

        # Store flip decisions in img_metas for consistency
        input_dict['pcd_horizontal_flip'] = pcd_horizontal_flip
        input_dict['pcd_vertical_flip'] = pcd_vertical_flip
